  ...
  return (
    <div className="container-responsive space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-3 rounded-xl border bg-emerald-50 dark:bg-emerald-950/40 border-emerald-100 dark:border-emerald-900 transition-colors">
          <span className="text-[11px] font-medium block text-emerald-700 dark:text-emerald-300">
          ...
          <span className="text-[10px] text-slate-500 dark:text-slate-400">
            {scope === 'overall' ? (
              <>
                Bank: {formatOMR(analyticsData.bankBalance)} · Cash:{' '}
                {formatOMR(analyticsData.cashBalance + analyticsData.pettyCashBalance)}
              </>
            ) : (
              <>Billing Progress: {formatPercent(analyticsData.billingProgress)}</>
            )}
          </span>
        </div>

        {/* KPI Card 2: Client Receivables & Billing */}
        <div className="p-3 rounded-xl border bg-blue-50 dark:bg-blue-950/40 border-blue-100 dark:border-blue-900 transition-colors">
          <span className="text-[11px] font-medium block text-blue-700 dark:text-blue-300">
          ...
          <span className="text-[10px] text-slate-500 dark:text-slate-400">
            Collected: {formatOMR(analyticsData.cumReceived)} · Eff: {formatPercent(analyticsData.collectionEfficiency)}
          </span>
        </div>

        {/* KPI Card 3: Vendor Payables / Budget Tracking */}
        <div className="p-3 rounded-xl border bg-amber-50 dark:bg-amber-950/40 border-amber-100 dark:border-amber-900 transition-colors">
          <span className="text-[11px] font-medium block text-amber-700 dark:text-amber-300">
          ...
          <span className="text-[10px] text-slate-500 dark:text-slate-400">
            {scope === 'overall' ? (
              <>
                Purchases: {formatOMR(analyticsData.cumPurchases)} · Direct Exp:{' '}
                {formatOMR(analyticsData.cumDirectExpenses)}
              </>
            ) : (
              <>
                Budget: {formatOMR(analyticsData.targetBudgetCost)} ·{' '}
        ...
        <div className="p-3 rounded-xl border bg-emerald-50 dark:bg-emerald-950/40 border-emerald-100 dark:border-emerald-900 transition-colors">
          ...
          <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold">
            Cost: {formatOMR(datePreset === 'all' ? analyticsData.cumTotalCost : analyticsData.periodTotalCost)} · Margin:{' '}
            {formatPercent(datePreset === 'all' ? analyticsData.cumMargin : analyticsData.periodMargin)}
          </span>
        </div>
      </div>
